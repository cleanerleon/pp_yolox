...
class YOLOXHead(nn.Layer):
    def __init__(
        ...
        self.grids = [paddle.zeros((1, 1))] * len(in_channels)

    def fill_bias(self, conv, size, value):
        # b = conv.bias.reshape(size)
        # b.stop_gradient = True
        # b.fill_(value)
        # b.stop_gradient = False
        # conv.bias = b
        bias_value = np.zeros(conv.bias.shape)
        bias_value.reshape(size).fill(value)
        # assert len(shape) == 1
        # bias_value = [value] * shape[0]
        # print(conv.bias.shape)
        # print(size, value)
        bias_attr = paddle.framework.ParamAttr(
            initializer=paddle.nn.initializer.Assign(bias_value))
        conv.bias = conv.create_parameter(
            attr=bias_attr, shape=bias_value.shape, is_bias=True)

    def initialize_biases(self, prior_prob):
        value = -math.log((1 - prior_prob) / prior_prob)
        size = [self.n_anchors, -1]
        for conv_list in (self.cls_preds, self.obj_preds):
            for conv in conv_list:
                self.fill_bias(conv, size, value)
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        if grid.shape[2:4] != output.shape[2:4]:
            ...
            grid = (
                paddle.stack((xv, yv), 2).reshape([1, 1, hsize, wsize, 2]).astype(dtype)
            )
            self.grids[k] = grid
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        grids = paddle.concat(grids, axis=1).type(dtype)
        strides = paddle.concat(strides, axis=1).type(dtype)
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            num_gts += num_gt
            if num_gt == 0:
                cls_target = paddle.zeros((0, self.num_classes), dtype)
                reg_target = paddle.zeros((0, 4), dtype)
                l1_target = paddle.zeros((0, 4), dtype)
                obj_target = paddle.zeros((total_num_anchors, 1), dtype)
                fg_mask = paddle.zeros([1, total_num_anchors], bool)
                ...
                if self.use_l1:
                    l1_target = self.get_l1_target(
                        paddle.zeros((num_fg_img, 4), dtype),
    ...
    def get_assignments(
        ...
        # with paddle.cuda.amp.autocast(enabled=False):
        #     cls_preds_ = (
