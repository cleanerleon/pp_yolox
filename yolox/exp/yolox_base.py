...
from ..data.data_augment import TrainTransform, ValTransform
...
class Exp(BaseExp):
    ...
    def get_data_loader(
        self, batch_size, no_aug=False, cache_img=False
    ):
        ...
        # Check https://github.com/pypaddle/pypaddle/issues/63311 for more details.
    ...
    def random_resize(self, data_loader, epoch):
        # tensor = paddle.LongTensor(2).cuda()
        tensor = paddle.to_tensor([0, 0], dtype='int64')
    ...
    def get_optimizer(self, batch_size):
        if "optimizer" not in self.__dict__:
            ...
            for k, v in self.model.named_sublayers():
                if hasattr(v, "bias") and isinstance(v.bias, Parameter):
                    pg2.append(v.bias)  # biases
                if isinstance(v, nn.BatchNorm2D) or "bn" in k:
                    pg0.append(v.weight)  # no decay
                elif hasattr(v, "weight") and isinstance(v.weight, Parameter):
            ...
            optimizer = paddle.optimizer.Momentum(
                learning_rate=lr, momentum=self.momentum, parameters=parameters, use_nesterov=True
            )
    ...
    def get_eval_loader(self, batch_size, testdev=False, legacy=False):
        ...
        sampler = paddle.io.SequenceSampler(valdataset)
